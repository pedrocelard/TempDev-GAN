...
class Generator_CLSTM(nn.Module):
    ...
    def forward(self, z, test=False, bio=False):
        ...
        if test:
            ...
            # render 1st
            b = self.sup1(h)
            x1 = self.buildr(h).view(N, T, self.colors, outsize // 8, outsize // 8)
            h, T = self.subsample(h, N, T)
            h = self.sup1(h)
            
            # render 2nd
            b = self.sup2(b)
            x2 = self.sup1r(h).view(N, T, self.colors, outsize // 4, outsize // 4)
            h, T = self.subsample(h, N, T)
            h = self.sup2(h)
            
            # render 3rd
            b = self.sup3(b)
            x3 = self.sup2r(h).view(N, T, self.colors, outsize // 2, outsize // 2)
            h, T = self.subsample(h, N, T)
            h = self.sup3(h)
            # render 4th
            b = self.sup3r(b).view(N, bT, self.colors, outsize, outsize).transpose(1, 2)
            
            x4 = self.sup3r(h).view(N, T, self.colors, outsize, outsize)

            return b, x1, x2, x3, x4 
